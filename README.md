...
테스트 스크립트를 로컬에서만 실행하는 것을 넘어, **GitHub Actions**를 통해 지속적 통합(CI) 환경을 구축했습니다.

- **자동 실행 트리거**: `main` 브랜치에 코드가 푸시(Push)되거나 풀 리퀘스트(PR)가 생성될 때마다 E2E 테스트가 자동으로 구동됩니다.
- **테스트 리포트 확인**: 테스트가 완료되거나 실패할 경우, GitHub 저장소의 **Actions** 탭 하단 `Artifacts` 항목에 Playwright HTML 리포트가 업로드됩니다. 이를 다운로드하여 실패 원인을 시각적으로(Trace, Video 등) 상세히 분석할 수 있습니다. 
